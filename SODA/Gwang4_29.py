...
# # 주성분 값의 차이를 검정 (독립표본 t-검정 사용)
# for i in range(other_dong_data.shape[1]):
#     statistic, p_value = ttest_ind(hwangyeongdong_data[:, i], other_dong_data[:, i])
...
# # 다른 동들의 주성분 값
# other_dong_data = np.concatenate((combined_pca_selected[0:1], combined_pca_selected[2:5], combined_pca_selected[8:9], combined_pca_selected[14:15]), axis=0)
...
# # 다른 동들의 주성분 값
# other_dong_data = np.concatenate((combined_pca_selected[1:2], combined_pca_selected[5:6], combined_pca_selected[7:8], combined_pca_selected[9:14]), axis=0)
...
# # 화양동의 주성분 값
# other_dong_data1 = np.concatenate((combined_pca_selected[0:1], combined_pca_selected[2:5], combined_pca_selected[8:9], combined_pca_selected[14:15]), axis=0)
#   # 7번째 인덱스

# # 다른 동들의 주성분 값
# other_dong_data2 = np.concatenate((combined_pca_selected[1:2], combined_pca_selected[5:6], combined_pca_selected[7:8], combined_pca_selected[9:14]), axis=0)

# # 주성분 값의 차이를 검정 (독립표본 t-검정 사용)
# for i in range(other_dong_data1.shape[1]):
#     statistic, p_value = ttest_ind(other_dong_data1[:, i], other_dong_data2[:, i])
#     print(f"주성분 {i+1}: t-statistic = {statistic:.2f}, p-value = {p_value:.4f}")
