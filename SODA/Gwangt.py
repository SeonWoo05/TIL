...
features3 = [features[5]] + features[7:9]
...
# KMeans 모델 학습
kmeans = KMeans(n_clusters=3, random_state=42)  # 엘보우 메소드의 결과로 6개의 클러스터를 설정 / 일관된 값을 얻기 위해 seed 지정
kmeans.fit(combined_pca_selected)
...
# 화양동의 데이터 가져오기
화양동_data = df.loc[['화양동'], features]  # Notice the double brackets to ensure DataFrame

# 전체 데이터에서 화양동 데이터 제외
전체_data = df.drop(index='화양동')[features]  # Using drop function to remove 화양동

# Hotelling's T-squared test 수행
def hotelling_t_squared_test(data1, data2):
    n1 = len(data1)
    n2 = len(data2)
    p = data1.shape[1]  # Use shape to get the number of columns (variables)
    
    # 각 데이터의 평균 벡터 계산
    mean1 = data1.mean().values.reshape(-1, 1)
    mean2 = data2.mean().values.reshape(-1, 1)
    
    # 각 데이터의 공분산 행렬 계산
    cov1 = data1.cov().values
    cov2 = data2.cov().values
    
    # 귀무가설 하에서의 Hotelling's T-squared 값 계산
    t_squared = float((n1 * n2) / (n1 + n2) * np.dot(np.dot((mean1 - mean2).T, np.linalg.inv((1/n1 + 1/n2) * (cov1 + cov2))), (mean1 - mean2)))
    ...
    # 귀무가설 하에서의 F 값 계산
    f_value = (df2 / (df1 * df2)) * (df2 / (df2 - 2)) * t_squared
...
# Hotelling's T-squared test 수행
t_squared, p_value = hotelling_t_squared_test(화양동_data, 전체_data)

# 결과 출력
print(f"Hotelling's T-squared: {t_squared:.2f}")
